...
//cada usuario debe tener un trozo aleatorio de cada arhcivo en su cache inicial - decentralizado
// Zi=(Wi,Wi,...,Wi)
void cache_inicial (std::string **archivo, int K, int N, int D, std::string ***cacheZ, int **mat_demanda, int *demanda, int *vector_comb)
{
	for (int k=0; k<K; k++)  //por cada usuario existente
    {
        int n = rand() % D;//elige el trozo al azar
...
//Funcion para generar las copias (caching) iniciales de los archivos por cada usuario
//cada usuario debe tener un trozo elegido de cada arhcivo en su cache inicial - centralizado
// Zi=(Wi,Wi,...,Wi)
...
for (int n = 0; n < N; n++)    //por cada archivo
{
	int j = 0;
	std::string cad;
	for (int i = 0; i < F; i=i+u)   //recortamos un trozo de tamaño u
	{
		printf("%d %d\n",i,j);
		if (F-i<u)
		{
...
//comienzo del envio de cache codficado decentralizado

//definición de variables
//definir la cantidad de archivos en la libreria (N)
int N = 4;
//definir el tamano de los archivos en la libreria (F) (bits) para el ejemplo seran bytes
int F = 60;
//definir la cantidad de usuarios (K)
int K = 4;
//establecer el tamaño/espacio de almacenamiento en la cache en cada nodo edge (M) debe ser ENTERO
float M = 2.0;
//se define t=KM/N como un valor ENTERO, que es un parametrode simetria para los subarchivos (verificar)
int t=K*M/N;
//cantidad de trozos a generar por archivo debiera ser D o j
int D;
D = nCr(K,t);
cout << "D: "<<D<<"\n";
//se define el tamaño de cada trozo (subarchivo) que se  tiene
int u=F/D;

//establecer el tamano de cache fraccional u=MF/N
//int u = M * F / N;

//estableceer el valor de la fracción Y de cada archivo (tamaño) ojo que es igual a t
float Y;
Y = M / N;
//printf ("Y=%f/%d=%f\n",M,N,Y);
...
printf("............\n");
int vector_comb[100], vector_comb_temp[100];
int j=0;
for (int w=1;w<=N;w++)
{
    int tt = nCr(K,w);
    comb_vector(K,w, vector_comb_temp);
for (int i = 0; i < tt; i++) {
        vector_comb[j+i] = vector_comb_temp[i];
    }
    j=j+tt;
}
for (int i=0;i<j;i++)
...
u=(int)F/(j+1);
D=j+1;
//Generar los trozos a enviar cada uno de u bytes
...
cache_inicial (trozo, K, N, D, cache3d_, vector_demanda,demanda,vector_comb);

//Muestra las copias de cada usuario.
Muestra_cache3d(cache3d_, vector_demanda, vector_comb,K, N, D);

// etapa de delivery - algoritmo decentralizado
...
for (int n=0; n<Q; n++)     //por cada indice de un arhivo
{
    for (int q=0;q<long_Q;q++)
    {
        ...
        printf("%c\n",elementoQ[n][q]);
        indiceQ.erase(std::remove(indiceQ.begin(),indiceQ.end(),elementoQ[n][q]),indiceQ.end()); //quitamos el indice respectivo
        ...
        //envioX[k][i] = archivo[f][n];//genera un vector con el trozo obtenido --- esto hay que revisar
